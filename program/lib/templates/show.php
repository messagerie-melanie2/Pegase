...
class Show {
  ...
  public static function Process() {
    ...
    // Ajout des labels
    o::add_label(array('Are you sure you want to delete the poll ?','Notify attendees','Are you sure you want to delete your response ?','Name already exists','Clic to validate this proposal','Clic to unvalidate this proposal','Clic to add this proposal to your calendar','This proposals is already in your calendar','Remove','Do you want to send a message to the attendees ?','Would you like to add responses to your calendar as tentative ?','Username','Email address','Yes','No','If needed','Adding prop to your calendar...','Load freebusy...','Would you like to delete tentatives events of this poll from your calendar ?','Deleting tentatives...','Tentatives correctly deleted','Remember to lock the poll when it\'s finished','Lock the poll'));
    ...
    if (\Program\Data\Poll::isset_current_poll() && \Program\Data\Poll::get_current_poll()->type == "date" && \Program\Data\User::isset_current_user() && \Program\Data\User::get_current_user()->user_id != \Program\Data\Poll::get_current_poll()->organizer_id && \Program\Data\Poll::get_current_poll()->locked == 1) {
      // Récupération des propositions validées
      self::$validate_proposals = \Program\Data\Poll::get_current_poll()->validate_proposals;
      $show_pop_up = false;
    ...
    if (\Program\Data\Poll::isset_current_poll() && \Program\Data\Poll::get_current_poll()->type == "date" && \Program\Data\User::isset_current_user()) {
      // Récupération des réponses du sondage
      if (! isset(self::$responses)) {
        self::$responses = \Program\Drivers\Driver::get_driver()->getPollResponses(\Program\Data\Poll::get_current_poll()->poll_id);
      }
