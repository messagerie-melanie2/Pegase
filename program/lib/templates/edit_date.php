 ...
 */
class Edit_date extends Template {
  /**
  ...
  public static function Process() {
    ...
    // Ajoute les invités
    o::set_env("attendees_list", unserialize(\Program\Data\Poll::get_current_poll()->attendees));
    // Définition du poll_uid pour un nouveau sondage
    o::set_env("poll_uid", \Program\Data\Poll::get_current_poll()->poll_uid);
    ...
    if (count(o::get_env("proposals")) >= o::get_env("nb_prop") && \Program\Data\Poll::get_current_poll()->type == "date") {
      if (o::get_env("mobile")) {
    ...
    // Ajout des labels
    o::add_label(['Monday','Tuesday','Wednesday','Thursday','Friday',
            'Saturday','Sunday','Mon','Tue','Wed','Thu','Fri','Sat','Sun',
            'January','February','March','April','May','June','July','August',
            'September','October','November','December','Jan','Feb','Mar','Apr',
            'May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','All day','Today',
            'Month','Week','Day','Start','End','Edit date (Y-m-d H:i:s)',
            'Edit date','Delete','Are you sure ? Not saved proposals are lost',
            'Choose date on the calendar','show calendar','hide calendar','None',
            'Tentative','Confirmed','Your freebusy','Loading your events...']);
    ...
    for ($i = 1; $i <= o::get_env("nb_prop"); $i ++) {
      if (! o::get_env("mobile")) {
        if (isset($proposals['edit_date' . $i])) {
          $attrib = array("id" => "edit_date$i","name" => "edit_date$i",
                  "class" => "edit_date");
          $attrib["type"] = "text";
          ...
          $html .= \Program\Lib\HTML\html::div(array(
                  "class" => "pure-control-group"), $input->show($proposals['edit_date' . $i]) . " " . \Program\Lib\HTML\html::a(array(
        ...
        $attrib_date_start = array("id" => "edit_date_start$i",
                "name" => "edit_date_start$i","class" => "edit_date_start",
                "type" => "date");
        $input_date_start = new \Program\Lib\HTML\html_inputfield($attrib_date_start);
        $attrib_date_end = array("id" => "edit_date_end$i",
                "name" => "edit_date_end$i","class" => "edit_date_end",
                "type" => "date");
        $input_date_end = new \Program\Lib\HTML\html_inputfield($attrib_date_end);
        $attrib_time_start = array("id" => "edit_time_start$i",
                "name" => "edit_time_start$i","class" => "edit_time_start",
                "type" => "time");
        $input_time_start = new \Program\Lib\HTML\html_inputfield($attrib_time_start);
        $attrib_time_end = array("id" => "edit_time_end$i",
                "name" => "edit_time_end$i","class" => "edit_time_end",
                "type" => "time");
        $input_time_end = new \Program\Lib\HTML\html_inputfield($attrib_time_end);

        $html .= \Program\Lib\HTML\html::div(array(
                "class" => "pure-control-group"), \Program\Lib\HTML\html::label(array(
                "for" => "edit_date_start$i"), Localization::g('Edit date (Y-m-d H:i:s)')) . " " . Localization::g("Start") . " " . $input_date_start->show(isset($date_start) ? $date_start : "") . $input_time_start->show(isset($time_start) ? $time_start : "") . " " . Localization::g("End") . " " . $input_date_end->show(isset($date_end) ? $date_end : "") . $input_time_end->show(isset($time_end) ? $time_end : ""));
        $html .= "<br>";
      }
    }
    return $html;
