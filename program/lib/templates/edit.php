...
class Edit extends Template {
  ...
  public static function Process() {
    ...
    if ((o::get_env("action") == ACT_MODIFY || o::get_env("action") == ACT_LOCK) && ! \Program\Data\Poll::isset_current_poll()) {
      o::set_env("page", "error");
  ...
  /**
   * Vérouillage et dévérouillage du sondage
   */
  public static function LockPoll() {
    if (! \Program\Data\Poll::isset_current_poll() || ! \Program\Data\User::isset_current_user()) {
      return;
    }
    // Vérouillage/dévérouillage du sondage
    if (o::get_env("action") == ACT_LOCK && \Program\Data\Poll::get_current_poll()->organizer_id == \Program\Data\User::get_current_user()->user_id && \Program\Data\Poll::get_current_poll()->locked == 0) {
    ...
    elseif (o::get_env("action") == ACT_UNLOCK && \Program\Data\Poll::get_current_poll()->organizer_id == \Program\Data\User::get_current_user()->user_id && \Program\Data\Poll::get_current_poll()->locked == 1) {
      $csrf_token = trim(strtolower(Request::getInputValue("_t", POLL_INPUT_GET)));
      if (Session::validateCSRFToken($csrf_token)) {
    ...
    if (isset($edit_title) && $edit_title != "") {
      if (Session::validateCSRFToken($csrf_token)) {
        ...
        if (o::get_env("action") == ACT_NEW) {
          $poll = new \Program\Data\Poll(array("title" => $edit_title,
        ...
        elseif (o::get_env("action") == ACT_MODIFY && \Program\Data\Poll::isset_current_poll() && \Program\Data\Poll::get_current_poll()->organizer_id == \Program\Data\User::get_current_user()->user_id) {
          // Modification du sondage courant
          $poll = \Program\Data\Poll::get_current_poll();
          // Déterminer si on doit notifier
          $send_notification = $poll->title != $edit_title || $poll->location != $edit_location || $poll->description != $edit_description;
