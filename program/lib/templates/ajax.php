  ...
  private static function add_tentative_calendar() {
    ...
    foreach ($proposals as $prop_key => $proposal) {
      if (is_array($prop_keys) && in_array($prop_key, $prop_keys)) {
        ...
        // La proposition n'est pas validée, il faut peut être la supprimer
        if (\Program\Lib\Event\Drivers\Driver::get_driver()->event_exists($proposals[$prop_key], (isset($events[$proposals[$prop_key]]) ? $events[$proposals[$prop_key]] : null), null, null, null, $cal)) {
          // L'événement existe, il faut donc le supprimer
