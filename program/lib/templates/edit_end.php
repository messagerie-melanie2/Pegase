 ...
 */
class Edit_end extends Template {
  ...
  public static function Process() {
    ...
    if (Session::validateCSRFToken($csrf_token)) {
      ...
      if (o::get_env("action") != ACT_NEW && \Program\Data\Poll::get_current_poll()->type == "date") {
      ...
      elseif (o::get_env("action") != ACT_NEW) {
      ...
      if ((o::get_env("action") == ACT_NEW
            || !$send_notification)
          && !\Program\Data\Poll::get_current_poll()->invit_sent
        ...
        o::set_env("attendees_list", $attendees);
        // Ajout des labels
        o::add_label(['This is not a valid email', 'Sending invitation...', 'Invitation has been sent']);
      }
    }
    else {
      o::set_env("page", "edit_" . \Program\Data\Poll::get_current_poll()->type);
  ...
  public static function GetPublicUrl($newtab = false) {
    ...
    $params = array(
            ...
            "class" => "public_url_link customtooltip_bottom","href" => $url);
    if ($newtab)
   ...
   *
   * @param array $proposals Liste des propositions supprimées
   */
