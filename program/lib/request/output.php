...
class Output {
	...
	public static function init() {
	    ...
	    if (isset($_GET['_skin'])
    			&& file_exists(__DIR__ . "/../../../skins/".Request::getInputValue("_skin", POLL_INPUT_GET))) {
	    ...
	    elseif (Request::isMobile()
	            && file_exists(__DIR__ . "/../../../skins/".\Config\IHM::$SKIN."_mobile")
	            && !Cookie::issetCookie("desktop_version")
	                || Cookie::issetCookie("mobile_version")) {
	    ...
	    if (empty($poll_uid)) $poll_uid = Request::getInputValue("poll_uid", POLL_INPUT_POST);
	 ...
	 * @param string $page [optionnal]
	 * @param string $action [optionnal]
	 * @param array $options [optionnal]
	 * @param boolean $escape_amp [optionnal]
	 */
	public static function url($page = null, $action = null, $options = array(), $escape_amp = true) {
	    $url = "";
	    // Définition de l'amp
	    $amp = '&';
	    if ($escape_amp) $amp = \Program\Lib\HTML\HTML::quote($amp);
	    // Gestion du login
	    if ($page == "login"
	            && isset(\Config\IHM::$LOGIN_URL)
	            && isset($options['url'])) {
	 ...
	 * @param string $url URL vers la requête à appeler
	 * @param string $action action de la commande
