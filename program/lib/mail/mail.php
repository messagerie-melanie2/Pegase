...
class Mail {
  ...
  public static function SendMail($from, $to, $subject, $bcc = null, $body = null, $body_html = null, $message = null, $message_id = null, $in_reply_to = null, $as_attachment = false) {
    ...
    if (isset($body_html)) {
      ...
      return mail($to, mb_encode_mimeheader(utf8_decode($subject)), $message, implode("\r\n", $headers), $envelopefrom);
    }
  ...
  public static function SendCreatePollMail(\Program\Data\Poll $poll, \Program\Data\User $user) {
    ...
    $body = str_replace("%%organizer_email%%", \Program\Data\User::get_current_user()->email, $body);
  ...
  /**
   * Méthode d'envoi du message d'invitation des participants
   *
   * @param \Program\Data\Poll $poll sondage créé
   * @param array $attendees Liste des invités
   * @return boolean
   */
  public static function SendInvitationMail(\Program\Data\Poll $poll, $attendees) {
    Log::l(Log::DEBUG, "Mail::SendInvitationMail()");
    $subject = Localization::g("Send invitation mail subject", false);
    $message_id = md5($poll->organizer_id . time() . "SendInvitationMail") . $poll->poll_uid . '-' . strtotime($poll->created) . "@" . \Config\IHM::$TITLE;
    $from = \Config\IHM::$FROM_MAIL;
    $proposals = unserialize($poll->proposals);
    $to = "undisclosed-recipients:;";
    $bcc = "";
    $body = file_get_contents(__DIR__ . '/templates/' . \Config\IHM::$DEFAULT_LOCALIZATION . '/invitation.html');

    // Replace elements
    $subject = str_replace("%%app_name%%", \Config\IHM::$TITLE, $subject);
    $subject = str_replace("%%poll_title%%", $poll->title, $subject);
    $subject = str_replace("%%organizer_shortname%%", self::UserShortName(\Program\Data\User::get_current_user()->fullname), $subject);
    ...
    return self::SendMail($from, $to, $subject, $bcc, null, null, $body, $message_id);
  }
