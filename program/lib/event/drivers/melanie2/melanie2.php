...
class Melanie2 extends \Program\Lib\Event\Drivers\Driver {
  ...
  public function get_user_timezone($user = null) {
    ...
    self::$timezone = new \DateTimeZone(self::$user->getTimezone());
    return self::$timezone;
  }
  ...
  public function add_to_calendar($date, $poll = null, $user = null, $status = null, $part_status = null, $calendar = null, $selected_date = false) {
    try {
      ...
      $this->init_event($date, $poll, $user, $status, $part_status, $selected_date);
  ...
  public function list_user_calendars($user = null) {
    ...
    foreach ($_calendars as $_cal) {
      if ($_cal->asRight(\LibMelanie\Config\ConfigMelanie::WRITE)
          && $_cal->id == $_cal->owner) {
   ...
   * @param boolean $selected_date Date retenue par l'organisateur ?
   */
  private function init_event($date, $poll, $user = null, $status = null, $part_status = null, $selected_date = false) {
