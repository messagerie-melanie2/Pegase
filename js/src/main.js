...
$(document).ready(function() {
  ...
  if (poll.env.max_own_polls) {
    if ($("#content #listyourpolls .poll__list_element").size() > poll.env.max_own_polls) {
  ...
  if (poll.env.max_deleted_polls) {
    if ($("#content #listyourdeletedpolls .poll__list_element").size() > poll.env.max_deleted_polls) {
    ...
    else if ($("#content #listyourdeletedpolls .poll__list_element").size() == 0 && $("#left-panel #listyourdeletedpolls .poll__list_element").size() == 0) {
      $("#listyourdeletedpolls").hide()
    }
  }
  // Affichage des sondages auxquels on répond
  if (poll.env.max_resp_polls) {
    if ($("#content #listpollsresponded .poll__list_element").size() > poll.env.max_resp_polls) {
